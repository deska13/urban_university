def send_email(
    message: str, recipient: str, *, sender: str = "university.help@gmail.com"
):
    if (
        "@" not in recipient
        or "@" not in sender
        or not any(recipient.endswith(i) for i in [".com", ".ru", ".net"])
        or not any(sender.endswith(i) for i in [".com", ".ru", ".net"])
    ):
